...
const ANIMATIONS: Record<string, (
    ...
    & ({ id: number; lib: "special" } | { id?: number; lib: string; }))> = {
...
export class AnimPctx extends Player.Context {
    ...
    public showAnimDialog(): void {
        ...
        this.dialog({
            id: 3,
            style: DialogStyles.TABLIST_HEADERS,
    ...
    public onDialogResponse(res: DialogResponse): void {
        ...
        if (res.item < 0 || res.item >= entries.length || !res.response) return;

        const [_key, anim] = entries[res.item];
        this.clearAnims();
        this.specialAction = 0;

        if (anim.lib === "special" && anim.id) {
    ...
    public onKeyStateChange(): void {
        ...
        if (Object.values(ANIMATIONS).find(a => a.lib == "special" && this.mode.specialAction == a.id))
            this.specialAction = 0;
    }
}
